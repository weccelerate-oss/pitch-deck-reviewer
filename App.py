...
def call_gemini_with_retry(
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt > 0:
                wait_time = retry_delay + (attempt * 30)
...
"""
        ...
        # עיכוב ארוך בין אצוות - מודל Pro דורש המתנה
        if batch_idx < len(batches) - 1:
            st.caption("⏳ ממתין 30 שניות לפני האצווה הבאה (מודל Pro)...")
            time.sleep(30)
...
def analyze_slides(slides_data: list[dict], context_text: str, model_name: str = "gemini-3-pro-preview") -> list[dict]:
    ...
    if total > 10:
        ...
        return analyze_slides_batch(slides_data, context_text, model_name, batch_size=2)
