...
/* קונטיינר הטבלה */
[data-testid="stDataEditor"] [data-testid="glide-data-editor"],
[data-testid="stDataFrame"] [data-testid="glide-data-editor"] {
    direction: rtl !important;
}
...
# System Prompt מקצועי לבדיקת מצגות סקירות שוק
SYSTEM_PROMPT = """
אתה בודק סקירות שוק מקצועי. תפקידך לבחון מצגות סקירה של מיזמים ולספק הערות מדויקות, ממוקדות וברורות.

═══════════════════════════════════════════════════════════════
עקרונות כלליים
═══════════════════════════════════════════════════════════════

• הקפד על בהירות, דיוק ורצף קריא.
• הטקסטים צריכים להיות קצרים ותמציתיים, ללא חזרות או מלל עודף.
• היצמד תמיד לשיחת הפתיחה של המיזם:
  - אם קיימת חוסר הלימה (קהל יעד, תחום פעילות, סוג פתרון) – ציין זאת במפורש.
  - דוגמה: "שקף 7 – חסרים מתחרים בתחום ___ שנזכר בשיחת הפתיחה, יש להוסיף לפחות שניים."

הבהרה לגבי קהלי יעד:
• כאשר מוצגים קהלי יעד נוספים מעבר לשיחת הפתיחה – אין לראות בכך טעות בהכרח, כל עוד ההרחבה הגיונית.
• הערה נדרשת רק אם קיימת סתירה או שינוי מהותי (למשל: בשיחת הפתיחה צוין B2B בלבד ובסקירה מוצג B2C).
• במקרים של הרחבה לגיטימית – ציין כהערת תיעוד: "הסקירה מוסיפה קהלי יעד נוספים – נראית הרחבה סבירה."
...
נתוני שוק:
• חייבים להיות עדכניים לשנתיים האחרונות בלבד (2024-2025).
• אין להציג נתונים מ-2022 ומטה.

נתוני מתחרים:
• ניתן להציג נתונים ישנים כאשר מדובר בנתונים קבועים (כגון שנת הקמה).
• לנתוני פעילות, פיילוטים או מחירים – חובה לציין הקשר ברור:
  - "מחיר בעת ההשקה ב-2019", "פיילוט ראשון ב-2020".

סדר מתחרים:
• המתחרים הישירים ביותר חייבים להופיע בתחילת הסקירה.
• אם מוצגים מתחרים פחות רלוונטיים בחלק העליון – העֵר ובקש להזיז את הישירים למעלה.
• חובה לכלול מתחרים מקומיים (ישראליים) אם קיימים בשוק. אם מופיעים רק מתחרים מחו"ל – העֵר על כך.

═══════════════════════════════════════════════════════════════
🧩 מתודולוגיית בדיקה ומתן הערות
═══════════════════════════════════════════════════════════════

• ציין תמיד את מספר השקף, הבעיה והפתרון.
  דוגמה: "שקף 5 – הנתונים ישנים מדי, עדכן ל-2024–2025."

• ההערות צריכות להיות מנוסחות כהנחיות ישירות, לא כהערות כלליות.

• אין לכתוב הערות על שקף המתודולוגיה – שקף זה מציג כותרות בלבד.

• אם הסקירה אינה מתאימה לקהל היעד (B2B מול B2C) – ציין זאת במפורש והצע התאמה.
...
הגדרת הבעיה והפתרון:
• כל אחת בשקף נפרד.
• מנוסחת בפסקה קצרה וטבעית (לא רשימת נקודות).
• ניסוח זורם וברור, גם בתחומים מורכבים.

הגדרת הבעיה:
• הצג את הבעיה באופן ענייני ומבוסס, ללא הדגשת בידול מול מתחרים.
• אין להשתמש בניסוחים מוחלטים כגון "כיום אין פתרון ש...".
• נסח בזהירות:
  - "קיימים פתרונות חלקיים, אך אף אחד מהם אינו מספק מענה מלא ל-___."
  - "הפתרונות הקיימים מתמקדים בעיקר ב-___, ואינם נותנים מענה ל-___."

הגדרת הפתרון:
• הצג את המענה באופן תיאורי וברור, תואם לשיחת הפתיחה.
• אין להדגיש בידול או יתרון תחרותי בשלב זה – רק לתאר את מהות הפתרון.

═══════════════════════════════════════════════════════════════
🌍 סקירת שווקים
═══════════════════════════════════════════════════════════════

• ודא שכל השווקים הרלוונטיים למיזם מוצגים.
• אין לכלול שווקים דומים מדי היוצרים עומס מיותר.
• חובה לכלול את השווקים הישירים והחשובים ביותר.
  דוגמה: אם המיזם עוסק בניטור שינה – לא מספיק להציג רק שוק הבריאות הדיגיטלית, יש לכלול גם שוק ניטור השינה.

עומס נתונים:
• אין להסתפק באמירה כללית על עומס – ציין במדויק מה להסיר:
  - "שקף 4 – הנתונים עמוסים מדי, מומלץ להסיר את שוק ___ ולהשאיר את ___ בלבד."
  - "שקף 6 – קיימת כפילות בין נתוני שוק ___ ו-___, השאר רק אחד מהם."

═══════════════════════════════════════════════════════════════
💬 שלב המסקנות
═══════════════════════════════════════════════════════════════

• הצג באופן ישיר את החסרונות של המתחרים ולצידם רעיונות לבידול.
• הפסקאות צריכות להיות קצרות, קריאות ולא ברשימות נקודות.
• אם חסרה התייחסות לבידול מול מתחרים – חובה להוסיף הערה:
  "שקף המסקנות – חסרה התייחסות למתחרים ולנקודות הבידול, יש להוסיף פסקה המסכמת את הפערים המרכזיים והיתרון של המיזם."

═══════════════════════════════════════════════════════════════
📋 הנחיות לסדר הבדיקה
═══════════════════════════════════════════════════════════════

• פעל אך ורק לפי הסדר הקיים במצגת – לא לפי מתודולוגיה קבועה מראש.
• כל הערה תיכתב בהתאם למספר השקף המופיע במצגת עצמה.
• אין להשתמש במספור פנימי (1, 2, 3...), אלא רק לציין את מספר השקף.
  דוגמה: "שקף 4 – הניסוח לא ברור, יש לחדד את הגדרת הבעיה."

הבדיקה תכלול:
• התאמה לשיחת הפתיחה (קהלי יעד, תחום, היגיון כללי).
• בהירות ורצף קריא בכל שקף.
• עדכניות נתונים (נתוני שוק לשנים 2024–2025 בלבד).
• רצף הגיוני בין השקפים.
• הערות ישירות וממוקדות, לא כלליות.

═══════════════════════════════════════════════════════════════
📤 פורמט פלט נדרש
═══════════════════════════════════════════════════════════════

החזר אך ורק JSON תקני המכיל רשימה של אובייקטים עבור כל שקף שדורש הערה.
כל אובייקט יכיל:

{
  "slide_number": מספר השקף (מספר שלם),
  "original_text": תקציר קצר של תוכן השקף (עד 100 תווים),
  "ai_comment": ההערה המקצועית שלך לפי הכללים הנ"ל. פורמט: "שקף X – [ההערה]". אם השקף תקין לחלוטין כתוב "תקין",
  "status": אחד מהבאים:
...
חשוב: אל תדלג על שקפים. החזר אובייקט לכל שקף במצגת.
"""
...
def call_gemini_with_retry(
    ...
    """
    קריאה ל-Gemini API עם מנגנון retry חכם.
    """
    ...
    for attempt in range(max_retries):
        try:
            if attempt > 0:
                # המתנה רק אם נכשלנו
                wait_time = 60 * attempt  # 60, 120 שניות
                st.warning(f"⏳ חריגה ממכסה. ממתין {wait_time} שניות ומנסה שוב ({attempt + 1}/{max_retries})...")
            ...
            if "429" in error_msg or "quota" in error_msg.lower():
                return None, "❌ חריגה ממכסת API - המתן דקה ונסה שוב"
            elif "403" in error_msg and "leaked" in error_msg.lower():
                return None, "❌ מפתח ה-API נחסם - צור מפתח חדש ב-Google AI Studio"
            elif "404" in error_msg:
                return None, f"❌ מודל לא נמצא"
            else:
                return None, f"❌ שגיאה: {error_msg[:100]}"
    
    return None, "❌ נכשל לאחר מספר ניסיונות"


def analyze_slides(slides_data: list[dict], context_text: str, model_name: str = "gemini-2.5-flash") -> list[dict]:
    """
    ניתוח כל השקפים בבקשה אחת - יעיל יותר!
    """
...
"""
    ...
    status = st.empty()
    status.markdown(f"🔍 **מנתח {total} שקפים עם {model_name}...**")
    ...
    for slide in slides_data:
        if slide["slide_number"] not in returned:
            result.append({
                ...
                "ai_comment": "⚠️ לא נותח",
                "status": "לביצוע"
            })
    
    st.success(f"✅ הניתוח הושלם! {len(result)} שקפים נבדקו.")
...
def main():
    ...
    with st.sidebar:
        ...
        model = st.selectbox(
            "בחירה",
            ["gemini-2.5-flash", "gemini-2.0-flash", "gemini-2.5-pro", "gemini-3-pro-preview"],
            label_visibility="collapsed"
        )
        model_desc = {
            "gemini-2.5-flash": "⚡ **מומלץ** - מהיר ויציב",
            "gemini-2.0-flash": "🚀 **מהיר** - מכסה גדולה",
            "gemini-2.5-pro": "🎯 **מדויק** - איכות גבוהה",
            "gemini-3-pro-preview": "🧠 **החכם** - מכסה מוגבלת"
        }
    ...
    if st.session_state.get("analysis_results"):
        ...
        results = sorted(results, key=lambda x: x.get("slide_number", 0))
        
        # סיכום סטטיסטי
        status_counts = {}
        for r in results:
            s = r.get("status", "לביצוע")
            status_counts[s] = status_counts.get(s, 0) + 1
        
        # כרטיסי סטטיסטיקה
        cols = st.columns(4)
        status_icons = {"לביצוע": "⏳", "נפתר": "✅", "אהבתי": "❤️", "למחוק": "🗑️"}
        status_colors = {"לביצוע": "#f59e0b", "נפתר": "#10b981", "אהבתי": "#ec4899", "למחוק": "#ef4444"}
        ...
        # טבלת הערות - תצוגה מותאמת RTL
        st.markdown("### 📝 הערות לפי שקף")
        st.markdown("")
        
        # עדכון סטטוס בצורה נוחה
        updated_results = []
        
        for idx, item in enumerate(results):
            slide_num = item.get("slide_number", "?")
            comment = item.get("ai_comment", "")
            original = item.get("original_text", "")[:150]
            current_status = item.get("status", "לביצוע")
            
            # כרטיס לכל שקף
            with st.container():
                st.markdown(f"""
                <div style="background: var(--bg-card, #1a1a2e); border: 1px solid #333; 
                            border-radius: 12px; padding: 1rem; margin-bottom: 1rem;
                            border-right: 4px solid {status_colors.get(current_status, '#6366f1')};">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem;">
                        <span style="font-size: 0.9rem; color: #94a3b8;">טקסט מקורי: {original}...</span>
                        <span style="background: {status_colors.get(current_status, '#6366f1')}33; 
                                     color: {status_colors.get(current_status, '#6366f1')}; 
                                     padding: 0.25rem 0.75rem; border-radius: 20px; font-size: 0.85rem;">
                            {status_icons.get(current_status, '📝')} שקף {slide_num}
                        </span>
                    </div>
                    <div style="color: #f8fafc; font-size: 1rem; line-height: 1.7; direction: rtl; text-align: right;">
                        {comment}
                    </div>
                </div>
                """, unsafe_allow_html=True)
                ...
                with col2:
                    new_status = st.selectbox(
                        ...
                        index=["לביצוע", "נפתר", "אהבתי", "למחוק"].index(current_status) if current_status in ["לביצוע", "נפתר", "אהבתי", "למחוק"] else 0,
                        key=f"status_{idx}",
                        label_visibility="collapsed"
                    )
        ...
        # שמירת השינויים
        st.session_state["analysis_results"] = updated_results
        ...
        # חישוב סטטיסטיקות
        results_for_download = st.session_state["analysis_results"]
        active_count = sum(1 for r in results_for_download if r.get("status") in ["לביצוע", "אהבתי"])
        todo_count = sum(1 for r in results_for_download if r.get("status") == "לביצוע")
        loved_count = sum(1 for r in results_for_download if r.get("status") == "אהבתי")
        
        # הצגת סיכום
        st.info(f"📊 **{active_count}** הערות יתווספו למצגת (לביצוע: {todo_count}, אהבתי: {loved_count})")
        ...
        with col2:
            if st.session_state.get("pptx_bytes"):
                try:
                    ...
                    st.error(f"❌ שגיאה ביצירת המצגת: {e}")
            else:
